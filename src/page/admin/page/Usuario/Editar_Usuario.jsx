...
const Editar_Usuario = () => {
   ...
   const { statusBotton, viewUsuario, status } = useSelector(
      (state) => state.user
   );
   const { startRol } = useRolStore();
   const { startGetIdUsuario, startPutUsuario } = useUserStore();
   const { id } = useParams();

   useEffect(() => {
      startGetIdUsuario(id);
      startRol();
   }, []);
   // console.log(viewUsuario);
   if (status === "Checking") {
      return <Spinner />;
   }
   ...
   return (
      <div className="animate__animated animate__fadeIn animate__faster m-5 mt-5 h-auto md:m-10">
         ...
         <Formik
            initialValues={{
               id: id,
               name: viewUsuario.name,
               email: viewUsuario.email,
               password: "",
               rol: viewUsuario.rol,
            }}
